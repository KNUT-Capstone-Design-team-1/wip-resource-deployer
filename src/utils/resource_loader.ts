...
import { RESOURCE_PROPERTY_MAP } from "./shared";
...
export class ResourceLoader {
  ...
  private async readFileContents(fileName: string): Promise<Array<TResource>> {
    ...
    switch (fileExtension) {
      ...
      case "pdf":
        return this.mappingProperty(
          await this.readPDFContents(fileName),
          RESOURCE_PROPERTY_MAP[dirName],
        );
   ...
   * @returns 페이지별 텍스트 배열
   */
  private async readPDFContents(filePath: string): Promise<string[]> {
  ...
  private mappingProperty(
    ...
    fileContents.forEach((resourceData) => {
      ...
      if (typeof resourceData === "string") {
        resource = this.mapStringResource(resourceData, normalizedPropertyMap);
      }

      if (typeof resourceData === "object" && resourceData !== null) {
        resource = this.mapObjectResource(resourceData, normalizedPropertyMap);
      }
  ...
  /**
   * 문자열 형태의 리소스 데이터를 매핑
   * @param resourceData 문자열 리소스 데이터
   * @param propertyMap 정규화된 프로퍼티 맵
   * @returns 매핑된 리소스 객체
   */
  private mapStringResource(
    resourceData: string,
    propertyMap: Record<string, string>,
  ): Record<string, any> {
    const resource: Record<string, any> = {};

    // propertyMap의 모든 value에 해당 내용을 매핑
    Object.values(propertyMap).forEach((targetProperty) => {
      resource[targetProperty] = resourceData;
    });
  ...
  /**
   * 객체 형태의 리소스 데이터를 매핑
   * @param resourceData 객체 리소스 데이터
   * @param propertyMap 정규화된 프로퍼티 맵
   * @returns 매핑된 리소스 객체
   */
  private mapObjectResource(
    ...
    Object.entries(resourceData).forEach(([key, value]) => {
      ...
      const targetProperty = propertyMap[normalizedKey];
